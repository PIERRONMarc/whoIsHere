...
//  WhoIsHere
//
//  Created by Etienne Vautherin on 11/02/2020.
...
//            if let email = ObservableModel.shared.email {
...
//            }
            ...
            AlwaysRespectful.monitor(elements: [insideBridge, outsideBridge])
                ...
                .sink(receiveCompletion: { (completion) in
                    ...
                    // set(inside: true, login: "Etienne@vautherin.com")
                    let model = ObservableModel.shared
                    if element.position == .inside {
                        startLocate()
                        model.inside = true
                        if let login = model.email {
    ...
    struct Images {
        ...
        static func upload(image: Image, with name: String) -> Future<URL, Error> {
            Future { (promise) in
